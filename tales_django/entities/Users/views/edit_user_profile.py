    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if user_form.is_valid():
                ...
                # logger.debug("Save user data: %s", user_data)
                user_form.save()
                ...
                if email_has_changed:
                    ...
                    # logger.debug("Email has changed: %s", debug_data)
                    send_re_actvation_email(request, user)
                    user.is_active = False
                    user.save()
                    messages.success(
                        request,
                        'Your email has been already changed. Now you have to re-activate your account. Please check your e-mail for an activation message.',
                    )
        ...
        return render(request, 'tales_django/profile_edit.html.django', context)
